...
<center><font size='4px'>View and delete appointment</font></center></div>
			...
			if(isset($_REQUEST['id']))
				
				{
					...
					echo"selected id's status has been updated";
			...
			<form action='checkbox_delete.php' method='post'>
			<div class='row'>
...
if($days>-1)
{
		...
		$where_con="where did=$did and date='$date' and status='0'";
			echo "Date:$date";
		...
		$query=mysqli_query($con,$sql);
				...
				if($num>0)
				{
					...
					echo"
					<form action='check_find.php' method='post'>
					<input type='hidden' name='date' value='$date'/>
					<input type='submit' value='checkbox delete' name='find1'/>";
					
					echo "<a href='checkbox_delete.php?date=$date&case=all' class='btn btn-info'>$num Delete All</a>";
					...
					//print_r($row_data);
					echo "<div class='row'>";
					echo"<input type='checkbox' name='chb[]' value='$row_data[id]'/>";
						for($i=0;$i<$col_num;$i++)
						{
	...
	echo "Previous record cannot be deleted ";
}			
"</div>";
echo"</div>";
echo"</div>";
