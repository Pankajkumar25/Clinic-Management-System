...
if(isset($_POST['update']))
{
    ...
    echo "<br/>";
    $specialist_value=$_POST['specialist'];
    ...
    $sql="update doctor set name='$name_value',address='$address_value',gender='$gender_value' where id=$id";
	echo "<br/>";
//	 echo $sql;
	// exit;
	if(mysqli_query($con,$sql))
	{
		...
		//header("Location:docprofile.php?pageno=$pageno&id=$id&caseno=1");
...
$sql="select id,name,address,gender,specialist from doctor WHERE id='$profile_id' ";
...
$sql1="select id,name,address,gender,specialist from doctor WHERE id='$profile_id' ";
...
echo "<form action='docprofile.php' method='post'>";
...
{echo "<tr>";
	...
	{
		echo "<form action='docprofile.php?pageno=$pageno' method='post' onsubmit='return confirm_msg(2,$id)'>";
		for($c=0;$c<count($col_list);$c++)
		{$col_name=$col_list[$c];
		if($col_name=='id')
		...
		echo"<td><a href='docprofile.php?pageno=$pageno'class='btn btn-info'>Cancel</a></td>";
	...
	echo"<td><a href='docprofile.php?pageno=$pageno&act=1&id=$row_data[id]' onclick='return edit_msg()' class='btn btn-info'>Edit</a></td>";
	echo"<td><a href='docprofile.php?pageno=$pageno&act=2&id=$row_data[id]'onclick='return confirm_msg(1,$row_data[id])' class='btn btn-danger'>Delete</a></td>";   //onclick='return confirm_msg(1,$id)'	
...
{
$lastpageno=$pageno-1;
echo "<a href='docprofile.php?pageno=$lastpageno' class='btn btn-info'>Previous</a>";
}
echo "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";

for($i=0,$j=1;$i<$total_pages;$i++,$j++)
{
	echo "<a href='docprofile.php?pageno=$i'>$j</a> &nbsp;&nbsp;";
}
...
{
$pageno++;
echo "<a href='docprofile.php?pageno=$pageno' class='btn btn-info'>Next</a>";
}
echo "</center>";
